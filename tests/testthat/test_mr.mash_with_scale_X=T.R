context("Test mr.mash with scale_X=T vs old mr.mash.scaled.X")

test_that("mr.mash with scale_X=T and old mr.mash.scaled.X return the same results", {
  ###Set seed
  set.seed(123)
  ...
  fit_current <- mr.mash(Y, X, V_est, S0mix, w0, tol=1e-8, update_w0=T, compute_ELBO=T, scale_X=T, verbose=F)
