...
test_that(paste("ELBO does not decrease over iterations with mixsqp weights",
  ...
  capture.output(
    fit <- mr.mash(X, Y, V_est, S0mix, w0, tol=1e-7, update_w0=TRUE,
                   update_w0_method="mixsqp", compute_ELBO=TRUE, 
                   standardize=FALSE, verbose=FALSE, update_V=FALSE,
                   version="R"))
  capture.output(
    fit_scaled <- mr.mash(X, Y, V_est, S0mix, w0, tol=1e-7, update_w0=TRUE,
                          update_w0_method="mixsqp", compute_ELBO=TRUE, 
                          standardize=TRUE, verbose=FALSE, update_V=FALSE,
                          version="R"))
  capture.output(
    fit_V <- mr.mash(X, Y, V_est, S0mix, w0, tol=1e-7, update_w0=TRUE,
                     update_w0_method="mixsqp", compute_ELBO=TRUE, 
                     standardize=FALSE, verbose=FALSE, update_V=TRUE,
                     version="R"))
  capture.output(
    fit_scaled_V <- mr.mash(X, Y, V_est, S0mix, w0, tol=1e-7, update_w0=TRUE,
                            update_w0_method="mixsqp", compute_ELBO=TRUE, 
  ...
  capture.output(
    fit_rcpp <- mr.mash(X, Y, V_est, S0mix, w0, tol=1e-7, update_w0=TRUE,
                        update_w0_method="mixsqp", compute_ELBO=TRUE, 
                        standardize=FALSE, verbose=FALSE, update_V=FALSE,
                        version="Rcpp"))
  capture.output(
    fit_scaled_rcpp <- mr.mash(X, Y, V_est, S0mix, w0, tol=1e-7,
                               update_w0=TRUE, update_w0_method="mixsqp",
                               compute_ELBO=TRUE, standardize=TRUE,
                               verbose=FALSE, update_V=FALSE, version="Rcpp"))
  capture.output(
    fit_V_rcpp <- mr.mash(X, Y, V_est, S0mix, w0, tol=1e-7, update_w0=TRUE,
                          update_w0_method="mixsqp", compute_ELBO=TRUE, 
                          standardize=FALSE, verbose=FALSE, update_V=TRUE,
                          version="Rcpp"))
  capture.output(
    fit_scaled_V_rcpp <- mr.mash(X, Y, V_est, S0mix, w0, tol=1e-7,
                                 update_w0=TRUE, update_w0_method="mixsqp",
                                 compute_ELBO=TRUE, standardize=TRUE,
