context("Test ELBO is non-decreasing over iterations with mixsqp weights updates")

test_that("ELBO does not decrease over iterations with mixsqp weights updates", {
  ###Set seed
  set.seed(123)
  ...
  fit <- mr.mash(Y, X, V_est, S0mix, w0, tol=1e-8, update_w0=TRUE, update_w0_method="mixsqp", compute_ELBO=TRUE, standardize=FALSE, verbose=FALSE)
  fit_scaled <- mr.mash(Y, X, V_est, S0mix, w0, tol=1e-8, update_w0=TRUE, update_w0_method="mixsqp", compute_ELBO=TRUE, standardize=TRUE, verbose=FALSE)
